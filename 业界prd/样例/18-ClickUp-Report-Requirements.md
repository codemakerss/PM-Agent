# 输出样例：Report Requirements｜Mixpanel Cohort 与 Funnel 分析：产品增长分析
...
Mixpanel 提供用户分群、漏斗、留存和行为分析，帮助团队识别转化和留存驱动因素。
...
| 能力 | 用户价值 | 需求表达 | 首版验收重点 |
|---|---|---|---|
| 用户分群 | 降低用户完成相关任务的操作成本 | 系统应支持“用户分群”相关核心流程，并在结果中展示状态、来源或可编辑内容 | 用户能在真实业务对象中完成端到端操作，且失败时有明确提示 |
| 漏斗分析 | 降低用户完成相关任务的操作成本 | 系统应支持“漏斗分析”相关核心流程，并在结果中展示状态、来源或可编辑内容 | 用户能在真实业务对象中完成端到端操作，且失败时有明确提示 |
| 留存分析 | 降低用户完成相关任务的操作成本 | 系统应支持“留存分析”相关核心流程，并在结果中展示状态、来源或可编辑内容 | 用户能在真实业务对象中完成端到端操作，且失败时有明确提示 |
| 导出与激活 | 降低用户完成相关任务的操作成本 | 系统应支持“导出与激活”相关核心流程，并在结果中展示状态、来源或可编辑内容 | 用户能在真实业务对象中完成端到端操作，且失败时有明确提示 |

## 3. 问题定义

### 3.1 用户问题

用户在处理“Mixpanel Cohort 与 Funnel 分析：产品增长分析”相关任务时，通常需要同时理解上下文、查找材料、判断下一步动作并与团队同步。如果没有产品化能力，用户会依赖手动复制、会议口头同步、个人经验或外部工具，容易造成信息遗漏、过程不可追踪和结果质量不稳定。

### 3.2 业务问题

对企业客户来说，用户分群、漏斗分析、留存分析、导出与激活 这类能力如果缺少权限、审计和配置机制，会带来数据泄露、误操作、标准不统一和成本不可控的问题。因此该功能不能只做成单点按钮，需要成为可治理、可衡量、可复盘的业务能力。
...
| 角色 | 核心诉求 | 关注风险 | 参与阶段 |
|---|---|---|---|
| 终端用户 | 快速完成 用户分群 和 漏斗分析 | 结果错误、入口难找、编辑困难 | 访谈、灰度、UAT |
...
| ID | 用户故事 | 验收标准 |
|---|---|---|
| US-01 | 作为终端用户，我希望直接在当前工作流中使用 用户分群。 | 入口清晰，用户无需离开主对象即可完成操作。 |
| US-02 | 作为终端用户，我希望系统支持 漏斗分析。 | 输出结果结构清楚，并能被复制、保存或分享。 |
| US-03 | 作为团队负责人，我希望结果能说明来源和生成依据。 | 结果展示来源、时间、触发条件或关联对象。 |
| US-04 | 作为管理员，我希望控制 留存分析 的可用范围。 | 管理页支持按团队、角色或空间配置。 |
| US-05 | 作为支持人员，我希望定位 导出与激活 相关失败原因。 | 日志记录错误码、输入对象、处理状态和反馈。 |

## 8. 专属功能需求

| ID | 专属功能需求 | 触发条件 | 输出/状态 | 优先级 |
|---|---|---|---|---|
| SFR-01 | 支持 用户分群 | 用户在对应工作对象中主动触发，或系统根据规则提示 | 生成可确认的结果，并保留来源、时间和处理状态 | P0 |
| SFR-02 | 支持 漏斗分析 | 用户在对应工作对象中主动触发，或系统根据规则提示 | 生成可确认的结果，并保留来源、时间和处理状态 | P0 |
| SFR-03 | 支持 留存分析 | 用户在对应工作对象中主动触发，或系统根据规则提示 | 生成可确认的结果，并保留来源、时间和处理状态 | P0 |
| SFR-04 | 支持 导出与激活 | 用户在对应工作对象中主动触发，或系统根据规则提示 | 生成可确认的结果，并保留来源、时间和处理状态 | P0 |
...
| 数据类型 | 用途 | 权限要求 | 风险控制 |
|---|---|---|---|
| 当前对象内容 | 作为 用户分群 的核心输入 | 继承对象权限 | 不展示无权限内容 |
| 用户身份与角色 | 判断功能可用性和可见范围 | 组织账号体系 | 记录操作人 |
| 历史操作记录 | 支持审计和质量分析 | 管理员/审计角色 | 保留日志和变更记录 |
| 外部连接器数据 | 支持 留存分析 或 导出与激活 | 需授权 | 显示连接状态和失败原因 |
...
- AC-01：给定用户拥有相关权限且输入资料完整，当用户触发“用户分群”时，系统应在合理时间内返回结构化结果，并允许用户确认、编辑或放弃。
- AC-02：给定用户拥有相关权限且输入资料完整，当用户触发“漏斗分析”时，系统应在合理时间内返回结构化结果，并允许用户确认、编辑或放弃。
- AC-03：给定用户拥有相关权限且输入资料完整，当用户触发“留存分析”时，系统应在合理时间内返回结构化结果，并允许用户确认、编辑或放弃。
- AC-04：给定用户拥有相关权限且输入资料完整，当用户触发“导出与激活”时，系统应在合理时间内返回结构化结果，并允许用户确认、编辑或放弃。
...
- 该案例的真实功能核心是：用户分群、漏斗分析、留存分析、导出与激活。
